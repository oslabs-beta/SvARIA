<script>
	...
	$: if (answer != '') {
		...
		popoverClassValue = `text-sm bg-zinc-100 text-black rounded-lg dark:bg-slate-800 dark:text-white border-orange-700 dark:border-white border-2 -translate-y-full -translate-x-8 p-4 w-fit overflow-y-auto h-${sizeValue}/6`;
	 } else {
		popoverClassValue =
