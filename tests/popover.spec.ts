...
test.describe('Popover', () => {
  ...
  test('closeButton renders with corrent content', async () => {
    await popover.click();
    const closeButton = popover.locator('button', { hasText: 'demoClose' });
    // await closeButton.click();
    expect(closeButton).toHaveCount(1);
  })

  test('closeButtonId', async () => {
    await popover.click();
    const closeButton = popover.locator('#demoCloseId');
    expect(closeButton).toHaveCount(1);
  })
